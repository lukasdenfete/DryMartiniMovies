...
public class ConnectionsViewModel : INotifyPropertyChanged
{
    ...
    public ObservableCollection<GraphSearchDto> UserHistory2 { get => _userHistory2; set { _userHistory2 = value; OnPropertyChanged(); } }
    ...
    private async void SearchHistory(ObservableCollection<GraphSearchDto> results, string query)
    {
        if (string.IsNullOrEmpty(query)) { results.Clear(); ShowHistory1 = false; ShowHistory2 = false; return; }
        var result = await _movieApiService.SearchGraphAsync(query);
        results.Clear();
        foreach (var r in result)
        {
            results.Add(r);
        }
        if (results == UserHistory1)
        {
            ShowHistory1 = true;
        }
        if (results == UserHistory2)
        {
            ShowHistory2 = true;
        }
    }
    ...
    {
        var dto = (GraphSearchDto)parameter;
        TmdbId1 = dto.TmdbId;
        _isSelecting = true;
        Movie1 = dto.Name;
        Label1 = dto.Label;
    ...
    private async void FindPath()
    {
        try {
        ErrorMessage = null;
        var path = await _movieApiService.FindShortestPathAsync(TmdbId1, TmdbId2, Label1, Label2);
        if (path == null)
